--[[
  ...
  Tabs remember last selection · Dropdown Vex-style (empty) · Support colors · Farm combos
]]
...
-- Live support with custom colors
local Support = Window:SupportStatus({
    Title = "Support",
    Status = "Online",
    Desc = "Staff online",
    Colors = {
        Online = Color3.fromRGB(34, 197, 94),
        Busy = Color3.fromRGB(249, 115, 22),
        Priority = { Color = Color3.fromRGB(168, 85, 247), Icon = "star" },
    },
})
Support:Bind(function()
    local p = { "Online", "Busy", "Away", "Online" }
    return { Status = p[math.random(1, #p)], Desc = os.date("%H:%M:%S") }
end, 10)
...
local Support = Window:Tab({ Title = "Support", Icon = "life-buoy" })
...
Support:Section({ Title = "Live Status" })
local TabSupport = Support:SupportStatus({
    Title = "Tickets",
    Status = "Online",
    Desc = "Avg wait 2m",
    Colors = {
        Online = Color3.fromRGB(16, 185, 129),
        Busy = Color3.fromRGB(245, 158, 11),
        Offline = Color3.fromRGB(100, 116, 139),
        Priority = { Color = Color3.fromRGB(192, 38, 211), Icon = "star" },
    },
})
Support:Dropdown({ Title = "Status", Option = { "Online", "Busy", "Away", "Offline", "Maintenance", "Connecting", "Priority" }, Callback = function(v)
    if not v then return end
    TabSupport:SetStatus(v)
    Support:SetStatus(v)
end })
Support:Colorpicker({ Title = "Custom Status Color", Default = Color3.fromRGB(34,197,94), Callback = function(c)
    TabSupport:SetColor(c)
    Support:SetColor(c)
end })
Support:Input({ Title = "Status Text / Desc", Placeholder = "message", Callback = function(t) TabSupport:SetDesc(t) Support:SetDesc(t) end })
Support:Button({ Title = "Set Priority + Purple", Callback = function()
    TabSupport:SetStatus("Priority", "star", Color3.fromRGB(192, 38, 211))
    Support:SetStatus("Priority", "star", Color3.fromRGB(192, 38, 211))
end })
Support:Button({ Title = "Hide / Show Float", Callback = function()
    local _, st = Support:Get()
