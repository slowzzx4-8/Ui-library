...
-- Icons com fallback (não quebra a library se o download falhar)
local IconsV2
do
    ...
    if ok and type(result) == "table" then
        IconsV2 = result
        pcall(function() IconsV2.SetIconsType("lucide") end)
    else
        warn("[Void Ui] Icons failed to load, using fallback")
        IconsV2 = {
            SetIconsType = function() end,
            GetIcon = function() return "" end,
            Icon = function() return false end,
        }
    end
end
...
local function GetIcon(icon)
    ...
    if not IconsV2 or not IconsV2.GetIcon then
        return ""
    end
    local ok, data = pcall(function()
        if typeof(icon) == "string" and icon:find(":") then
            ...
            if pack and name then
                pcall(function() IconsV2.SetIconsType(string.lower(pack)) end)
...
function UI:CreateWindow(Config)
    ...
    function Window:Tab(Config, type)
       ...
       local function SelectTab()
            UI:CloseActiveDropdown()
            -- grava índice da aba (persiste no minimize/reabrir)
            for _idx, _t in ipairs(Window.TabList or {}) do
                if _t == Tab then
                    Window.SelectedTabIndex = _idx
                    Window._SavedTabIndex = _idx
                    break
                end
            end
    ...
    function Window:Open()
        ...
        -- restaura última aba (não força a 1ª se já havia outra)
        local idx = tonumber(Window._SavedTabIndex) or tonumber(Window.SelectedTabIndex) or 1
    ...
    function Window:Close()
        ...
        -- salva aba + transparência
        Window._SavedTabIndex = tonumber(Window.SelectedTabIndex) or tonumber(Window._SavedTabIndex) or 1
        Window.SelectedTabIndex = Window._SavedTabIndex
