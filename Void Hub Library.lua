...
function Utility:Element(RightScroll, ElementFrame, sizeY, scope)
    local width = 0
    if ElementFrame and ElementFrame.AbsoluteSize and ElementFrame.AbsoluteSize.X > 20 then
        width = ElementFrame.AbsoluteSize.X - 16
    elseif ElementFrame and ElementFrame.Size then
        width = math.max(100, (ElementFrame.Size.X.Offset or 200) - 10)
    else
        width = 200
    end
    ...
    local Beeee = VoidUI:Create("Frame", {
        ...
        AutomaticSize = Enum.AutomaticSize.Y,
        Size = UDim2.new(1, -8, 0, sizeY or 44),
        ZIndex = 15,
    })
...
function UI:CreateWindow(Config)
    ...
    -- Botão open/close: quadrado preto pequeno no lado direito
    local function getSafeOpenButtonPos()
        return UDim2.new(1, -56, 0.5, 0)
    end

    local OpenButtonHolder = VoidUI:Create("Frame", {
        Parent = UIScreen,
        Size = UDim2.new(0, 44, 0, 44),
        Position = getSafeOpenButtonPos(),
        AnchorPoint = Vector2.new(0.5, 0.5),
        BackgroundTransparency = 1,
        Visible = false,
        ZIndex = 200,
    ...
    -- Arrastar o botão (qualquer área)
    ...
    do
        ...
        UserInputService.InputChanged:Connect(function(input)
            if dragging and (input.UserInputType == Enum.UserInputType.MouseMovement or input.UserInputType == Enum.UserInputType.Touch) then
                ...
                OpenButtonHolder.Position = UDim2.new(0, startAbs.X + delta.X, 0, startAbs.Y + delta.Y)
            end
        end)
    end
    ...
    local OpenBtnTitle = { Text = Window.Name } -- compat (não usa título no botão quadrado)
    ...
    function Window:EditOpenButton(Config)
        ...
        local Edit = {
            ...
            Image = Config.Image or Config.IconImage or nil,
            Transparency = Config.Transparency,
            CornerRadius = Config.CornerRadius or UDim.new(0, 10),
            Size = Config.Size or 44,
            Color = Config.Color,
        }
    ...
    local Main = VoidUI:Create("Frame", {
        ...
        AnchorPoint = Vector2.new(0.5, 0.5),
        Position = UDim2.new(0.5, 0, 0.5, 0), -- centro estável (independente de DPI)
    ...
    function Window:Tab(Config, type)
        ...
        local TabIcon
        if Tab.Icon then
            local _tabIconData = GetIcon(Tab.Icon)
            local _tabIconImg = ResolveIconImage(Tab.Icon)
            ...
            if typeof(_tabIconData) == "table" then
                if _tabIconData.ImageRectOffset then TabIcon.ImageRectOffset = _tabIconData.ImageRectOffset end
                if _tabIconData.ImageRectSize then TabIcon.ImageRectSize = _tabIconData.ImageRectSize end
            end
        end
        ...
        local ElementFrame = VoidUI:Create("Frame", {
            ...
            AnchorPoint = Vector2.new(1, 0),
            Position = UDim2.new(1, -5, 0, Window.Topbar.Height + 13),
            BorderColor3 = Color3.new(0, 0, 0),
            ClipsDescendants = true,
            BackgroundTransparency = 0.2,
            Visible = false,
            Size = UDim2.new(0, math.max(120, Window.Size.X.Offset - Window.SideBarWidth - 13), 0, math.max(80, Window.Size.Y.Offset - Window.Topbar.Height - 20)),
            BackgroundColor3 = Color3.fromRGB(33, 33, 33),
            ZIndex = 4,
            ThemeID = {
                BackgroundColor3 = "SideBar"
        ...
        RightScroll.UIListLayout:GetPropertyChangedSignal("AbsoluteContentSize"):Connect(function()
            local h = RightScroll.UIListLayout.AbsoluteContentSize.Y + 16
            RightScroll.CanvasSize = UDim2.new(0, 0, 0, h)
        end)
       local function SelectTab()
            UI:CloseActiveDropdown()
            local contentW = math.max(120, Window.Size.X.Offset - Window.SideBarWidth - 13)
            local contentH = math.max(80, Window.Size.Y.Offset - Window.Topbar.Height - 20 - ((Tags or 0) > 0 and 37 or 0))
            for _, v in next, ElementFolder:GetChildren() do
            ...
            ElementFrame.Size = UDim2.new(0, contentW, 0, contentH)
            ElementFrame.BackgroundTransparency = 0.2
            ElementFrame.Position = UDim2.new(1, -5, 0, Window.Topbar.Height + 13)
            -- só ajusta LARGURA; preserva altura / AutomaticSize
            for _, item in ipairs(RightScroll:GetChildren()) do
                if item:IsA("GuiObject") and not item:IsA("UIListLayout") and not item:IsA("UIPadding") then
                    local auto = item.AutomaticSize
                    local keepY = item.Size.Y
                    if auto == Enum.AutomaticSize.Y or auto == Enum.AutomaticSize.XY then
                        item.Size = UDim2.new(0, contentW - 10, 0, 0)
                    else
                        item.Size = UDim2.new(0, contentW - 10, keepY.Scale, keepY.Offset)
                    end
                end
            end
            -- força refresh do canvas
            if RightScroll.UIListLayout then
                RightScroll.CanvasSize = UDim2.new(0, 0, 0, RightScroll.UIListLayout.AbsoluteContentSize.Y + 16)
            end
        ...
        -- primeira aba abre automaticamente (evita painel vazio)
        ...
        function Tab:Paragraph(Config,type)
            ...
            local ParagraphFrame = VoidUI:Create("Frame", {
                ...
                ClipsDescendants = true,
                BackgroundTransparency = 0.5,
                Size = UDim2.new(1, -8, 0, Paragraph.SizeY),
                BackgroundColor3 = ResolvedColor or Color3.fromRGB(43, 43, 43),
                BorderSizePixel = 0,
                ZIndex = 15,
        ...
        function Tab:Dropdown(Config)
            -- Dropdown estilo source.lua (lista lateral + indicador), visual Void UI
            local Dropdown = {
            ...
            if Dropdown.Multi then
                if type(Dropdown.Value) == "table" then
                    Dropdown.Options = table.clone and table.clone(Dropdown.Value) or {unpack(Dropdown.Value)}
            ...
            local DropFrame = VoidUI:Create("Frame", {
                ...
                AutomaticSize = Enum.AutomaticSize.Y,
                ZIndex = 15,
                ThemeID = { BackgroundColor3 = "Dropdown.Background|ElementColor" },
            }, {
            ...
            local Title = Utility:ElText(DropFrame, Dropdown.Title, Dropdown.Desc, "Dropdown")

            local SelectOptionsFrame = VoidUI:Create("Frame", {
                Parent = DropFrame,
                AnchorPoint = Vector2.new(1, 0),
            ...
            local function displayText()
                if Dropdown.Multi then
                    local t = table.concat(Dropdown.Options, ", ")
                    return (t ~= "" and t) or "Select Options"
                end
                local v = Dropdown.Value
                if type(v) == "table" then v = table.concat(v, ", ") end
                return (v and tostring(v) ~= "" and tostring(v)) or "Select Options"
            end

            local OptionSelecting = VoidUI:Create("TextLabel", {
            ...
            local OptionImg = VoidUI:Create("ImageLabel", {
                Parent = SelectOptionsFrame,
                AnchorPoint = Vector2.new(1, 0.5),
            ...
            local DropBtn = VoidUI:Create("TextButton", {
                Parent = DropFrame,
                Size = UDim2.new(1, 0, 1, 0),
                BackgroundTransparency = 1,
                Text = "",
            ...
            local function closeDropdown()
                ...
                pcall(function() OptionImg.Rotation = 0 end)
            ...
            local function rebuildOptions()
                ...
                local accent = (UI.Theme and UI.Theme.Outline) or Color3.fromRGB(80, 160, 255)
                local elCol = (UI.Theme and UI.Theme.ElementColor) or Color3.fromRGB(40, 40, 44)
                local txtCol = (UI.Theme and UI.Theme.Text) or Color3.fromRGB(230, 230, 230)

                for idx, OptionName in ipairs(Dropdown.Option) do
                    local selected = isSelected(OptionName)
                    local Option = VoidUI:Create("Frame", {
                    ...
                    local ChooseFrame = VoidUI:Create("Frame", {
                        Parent = Option,
                        AnchorPoint = Vector2.new(0, 0.5),
                    ...
                    local OptionText = VoidUI:Create("TextLabel", {
                        Parent = Option,
                    ...
                    local OptionButton = VoidUI:Create("TextButton", {
                        Parent = Option,
                        Size = UDim2.new(1, 0, 1, 0),
                        BackgroundTransparency = 1,
                        Text = "",
                    ...
                    OptionButton.MouseButton1Click:Connect(function()
                        ...
                        if Dropdown.Multi then
                            ...
                            for i = #Dropdown.Options, 1, -1 do
                                if Dropdown.Options[i] == OptionName then
                            ...
                            if not found then
                                table.insert(Dropdown.Options, OptionName)
                            end
                            Dropdown.Value = Dropdown.Options
                            ...
                            OptionSelecting.Text = displayText()
                            task.spawn(Dropdown.Callback, OptionName)
            ...
            local function openDropdown()
                ...
                local menuW = 168
                local count = math.max(1, #Dropdown.Option)
                local menuH = math.clamp(count * 33 + 12, 80, 220)

                FloatingMenu = VoidUI:Create("Frame", {
                    Parent = Main,
                    Name = "VoidDropdownMenu",
                    Size = UDim2.new(0, menuW, 0, menuH),
                    BackgroundColor3 = (UI.Theme and UI.Theme.Background) or Color3.fromRGB(18, 18, 20),
                    ...
                    VoidUI:Create("UICorner", { CornerRadius = UDim.new(0, 10) }),
                    VoidUI:Create("UIStroke", {
                        Color = (UI.Theme and UI.Theme.Outline) or Color3.fromRGB(50, 50, 55),
                        Thickness = 1,
                        Transparency = 0.3,
                    }),
                })

                -- posiciona à direita do campo SelectOptionsFrame, dentro da Main
                task.defer(function()
                    if not FloatingMenu or not DropOpen then return end
                    local mainPos = Main.AbsolutePosition
                    local mainSize = Main.AbsoluteSize
                    local trigger = SelectOptionsFrame.AbsolutePosition
                    local triggerSize = SelectOptionsFrame.AbsoluteSize
                    local relX = (trigger.X + triggerSize.X - menuW) - mainPos.X
                    local relY = (trigger.Y + triggerSize.Y + 6) - mainPos.Y
                    relX = math.clamp(relX, 6, math.max(6, mainSize.X - menuW - 6))
                    if relY + menuH > mainSize.Y - 6 then
                        relY = math.max(6, (trigger.Y - mainPos.Y) - menuH - 6)
                    end
                    FloatingMenu.Position = UDim2.new(0, relX, 0, relY)
                end)
            ...
            UserInputService.InputBegan:Connect(function(input)
                if not DropOpen or selectingItem then return end
                if input.UserInputType ~= Enum.UserInputType.MouseButton1 and input.UserInputType ~= Enum.UserInputType.Touch then return end
                if tick() - openTick < 0.2 then return end
                task.delay(0.05, function()
                    if not DropOpen or not FloatingMenu or selectingItem then return end
                    local pos = input.Position
                    local ap, as = FloatingMenu.AbsolutePosition, FloatingMenu.AbsoluteSize
                    local inside = pos.X >= ap.X - 4 and pos.X <= ap.X + as.X + 4 and pos.Y >= ap.Y - 4 and pos.Y <= ap.Y + as.Y + 4
                    local vp, vs = DropFrame.AbsolutePosition, DropFrame.AbsoluteSize
                    local insideDrop = pos.X >= vp.X and pos.X <= vp.X + vs.X and pos.Y >= vp.Y and pos.Y <= vp.Y + vs.Y
                    if not inside and not insideDrop then
                        closeDropdown()
                    end
                end)
            end)
            ...
            DropBtn.MouseButton1Click:Connect(function()
                if Dropdown.Locked then return end
                if DropOpen then
                    if tick() - openTick < 0.15 then return end
                    closeDropdown()
                else
                    openDropdown()
                end
            ...
            function Dropdown:Clear()
                Dropdown.Options = {}
                Dropdown.Value = Dropdown.Multi and {} or ""
                OptionSelecting.Text = "Select Options"
                if DropOpen then rebuildOptions() end
                return Dropdown
            end

            function Dropdown:AddOption(name)
                table.insert(Dropdown.Option, name)
                if DropOpen then rebuildOptions() end
                return Dropdown
            end

            function Dropdown:Refresh(list, selecting)
                Dropdown.Option = list or Dropdown.Option
                if selecting ~= nil then
                    if Dropdown.Multi then
                        Dropdown.Options = type(selecting) == "table" and selecting or { selecting }
                        Dropdown.Value = Dropdown.Options
                    else
                        Dropdown.Value = type(selecting) == "table" and selecting[1] or selecting
                    end
                end
                OptionSelecting.Text = displayText()
                if DropOpen then rebuildOptions() end
            ...
            function Dropdown:Set(value)
                if Dropdown.Multi then
                    Dropdown.Options = type(value) == "table" and value or { value }
                    Dropdown.Value = Dropdown.Options
                else
                    Dropdown.Value = type(value) == "table" and value[1] or value
                end
                OptionSelecting.Text = displayText()
                if DropOpen then rebuildOptions() end
                return Dropdown
            end
            function Dropdown:SetValue(v) return Dropdown:Set(v) end
            function Dropdown:Get()
                return Dropdown.Multi and Dropdown.Options or Dropdown.Value
            end
            function Dropdown:Close()
                closeDropdown()
                DropDownElement:Destroy()
            end
            function Dropdown:SetTitle(v)
                if Title and Title.SetText then Title.SetText(v) end
            end

            OptionSelecting.Text = displayText()
            Utility:Search(Window, {
                Title = Dropdown.Title, Desc = Dropdown.Desc, Icon = "list",
                Type = "Dropdown", TabTitle = Tab.Title, SelectFn = SelectTab,
                Frame = DropDownElement, RightScroll = RightScroll,
            })
    ...
    function Window:Close()
        ...
        -- botão sempre no lado direito
        OpenButtonHolder.AnchorPoint = Vector2.new(0.5, 0.5)
        OpenButtonHolder.Position = getSafeOpenButtonPos()
    ...
    function Window:ToCenter()
        Utility:TweenObject(Main, {Position = UDim2.new(0.5, 0, 0.5, 0)}, 0.15, Enum.EasingStyle.Quint, Enum.EasingDirection.Out)
    end
    ...
    function Window:Resize(sizeX, sizeY, isDrag)
        ...
        for _, EF in ipairs(ElementFolder:GetChildren()) do
            if EF:IsA("Frame") then
                ...
                if Scroll then
                    local iw = sizeX - Window.SideBarWidth - 8 - 10
                    for _, item in ipairs(Scroll:GetChildren()) do
                        if item:IsA("GuiObject") and not item:IsA("UIListLayout") and not item:IsA("UIPadding") then
                            local auto = item.AutomaticSize
                            if auto == Enum.AutomaticSize.Y or auto == Enum.AutomaticSize.XY then
                                item.Size = UDim2.new(0, iw, 0, 0)
                            else
                                item.Size = UDim2.new(0, iw, item.Size.Y.Scale, item.Size.Y.Offset)
                            end
                        end
                    end
                end
    ...
    function Window:ToggleFullscreen()
        if Window.IsFullscreen then
            ...
            -- sempre centro
            Main.AnchorPoint = Vector2.new(0.5, 0.5)
            Utility:TweenObject(Main, { Position = UDim2.new(0.5, 0, 0.5, 0) }, 0.25, Enum.EasingStyle.Quint, Enum.EasingDirection.Out)
