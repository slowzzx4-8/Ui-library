...
function UI:CreateWindow(Config)
    ...
    function Window:SetTheme(themeName)
        ...
        -- atualiza toggles e elementos com RefreshTheme
    ...
    local OpenButtonHolder = VoidUI:Create("Frame", {
        ...
        Position = UDim2.new(0.5, -29, 0.5, -241),
        AnchorPoint = Vector2.new(0.5, 0.5),
        BackgroundTransparency = 1,
        Visible = false,
        ZIndex = 200,
    ...
    function Window:SetBackgroundImage(image, transparency)
        ...
        -- imagem/vídeo mais visível: Main bem transparente
        Utility:TweenObject(Main, { BackgroundTransparency = math.clamp(0.65 + t * 0.25, 0.55, 0.92) }, 0.2)
        BackgroundImage.ImageTransparency = 1
        BackgroundImage.ZIndex = 0
        BackgroundLayer.ZIndex = 0
        Utility:TweenObject(BackgroundImage, { ImageTransparency = math.clamp(t * 0.7, 0.05, 0.55) }, 0.25, Enum.EasingStyle.Quint, Enum.EasingDirection.Out)
        return Window
    end
        ...
        function Tab:Dropdown(Config)
            ...
            local function rebuildItemsInto(parentScroll)
                ...
                for idx, Item in ipairs(items) do
                    ...
                    local elCol = (UI.Theme and UI.Theme.ElementColor) or Color3.fromRGB(28, 28, 30)
                    ...
                    local label = VoidUI:Create("TextLabel", {
                        ...
                        BackgroundTransparency = 1,
                        Text = tostring(Item),
                        TextTransparency = selected and 0 or 0.28,
                        TextSize = 13,
                        FontFace = Font.new([[rbxassetid://12187365364]], Enum.FontWeight.SemiBold),
                        TextColor3 = txtCol,
                        TextXAlignment = Enum.TextXAlignment.Left,
                    ...
                    local function pick()
                        ...
                        selectingItem = true
                        -- highlight no clique; some em 1s (se soltar)
                        Utility:TweenObject(row, { BackgroundTransparency = 0 }, 0.08)
                        task.delay(1, function()
                            if not selectingItem and row and row.Parent then
                                local stillSel = false
                                if Dropdown.Multi then
                                    for _, ov in ipairs(Dropdown.Options or {}) do
                                        if ov == Item then stillSel = true break end
                                    end
                                else
                                    stillSel = (Dropdown.Value == Item)
                                end
                                if not stillSel then
                                    Utility:TweenObject(row, { BackgroundTransparency = 0.55 }, 0.2)
                                end
                            end
                        end)
                        if not Dropdown.Multi then
                            ...
                            local elC = (UI.Theme and UI.Theme.ElementColor) or Color3.fromRGB(28, 28, 30)
                            for _, r in ipairs(parentScroll:GetChildren()) do
                                if r:IsA("TextButton") then
                                    ...
                                    if lab then
                                        ...
                                        r.BackgroundColor3 = elC
                                        lab.TextTransparency = isSel and 0 or 0.28
                                    end
                                end
                            end
                    ...
                    row.MouseButton1Down:Connect(function()
                        selectingItem = true
                        Utility:TweenObject(row, { BackgroundTransparency = 0 }, 0.06)
                    end)
            ...
            local function placeMenuInsideMain(menu, menuW, menuH)
                ...
                -- relativo à Main para seguir a UI
                local relX = (trigger.X + triggerSize.X - menuW) - mainPos.X
                local relY = (trigger.Y + triggerSize.Y + 4) - mainPos.Y
                relX = math.clamp(relX, 6, math.max(6, mainSize.X - menuW - 6))
                -- se não couber embaixo, abre para cima
                if relY + menuH > mainSize.Y - 8 then
                    relY = (trigger.Y - mainPos.Y) - menuH - 4
                end
                relY = math.clamp(relY, 6, math.max(6, mainSize.Y - menuH - 6))
            ...
            local function openDropdown()
                ...
                task.defer(function()
                    ...
                    local rs = RightScroll
                    local rsW = (rs and rs.AbsoluteSize.X) or 280
                    local mainSize = Main and Main.AbsoluteSize or Vector2.new(480, 360)
                    local menuW = math.clamp(rsW - 16, 160, mainSize.X - 24)
                    local menuH = math.min(math.max(#(Dropdown.Option or {}) * 32 + 48, 90), math.min(220, mainSize.Y - 24))
                    ...
                    FloatingMenu = VoidUI:Create("Frame", {
                        ...
                        BackgroundTransparency = Window.Transparent and 0.08 or 0.02,
                        ZIndex = 900,
                        ClipsDescendants = true,
                        Active = true,
                    }, {
                    ...
                    local searchRow = VoidUI:Create("Frame", {
                        ...
                        BackgroundTransparency = Window.Transparent and 0.35 or 0.2,
                        ZIndex = 901,
                    }, {
                        VoidUI:Create("UICorner", { CornerRadius = UDim.new(0, 8) }),
                    })
    ...
    function Window:Close()
        ...
        task.delay(0.22, function()
            ...
            OpenButtonHolder.Position = UDim2.new(0.5, -29, 0.5, -241)
