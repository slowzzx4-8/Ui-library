...
function UI:CreateWindow(Config)
    ...
    local Main = VoidUI:Create("Frame", {
        ...
        AnchorPoint = Vector2.new(0.5, 0.5),
        Position = UDim2.new(0.5, 0, 0, 48 + math.floor(Window.Size.Y.Offset / 2)), -- bem no topo
    ...
    WindowDragBar.MouseEnter:Connect(function()
        Utility:TweenObject(WindowDragBar, {BackgroundTransparency = 0.35}, 0.1)
    end)
    WindowDragBar.MouseLeave:Connect(function()
        Utility:TweenObject(WindowDragBar, {BackgroundTransparency = 0.8}, 0.15)
    end)
       ...
       local function SelectTab()
        ...
        function Tab:Toggle(Config,type)
            ...
            local ToggleV = VoidUI:Create("Frame", {
                ...
                ZIndex = 15,
                ThemeID = {
                    BackgroundColor3 = "Toggle.Track|Background"
                }
            },{
                VoidUI:Create("Frame", {
                    Name = "Knob",
                    ...
                    ZIndex = 16,
                    ThemeID = {
                        BackgroundColor3 = "Toggle.Knob|Text"
                    }
                },{
                    VoidUI:Create("UICorner", {
            ...
            function Togglee:SetValue(newValue)
                ...
                if newValue then
                    Utility:TweenObject(ToggleV, {BackgroundColor3 = Color3.fromRGB(55, 55, 55)}, 0.2)
                    ...
                    Utility:TweenObject(ToggleV, {BackgroundColor3 = UI.Theme.Background or Color3.fromRGB(20, 20, 20)}, 0.2)
                    Utility:TweenObject(knob, {
                        Position = UDim2.new(0, 12, 0.5, 0),
        ...
        function Tab:Dropdown(Config)
            ...
            -- Dropdown flutuante (estilo Main.lua) — não expande o layout / não buga o scroll
            ...
            local SearchBox = nil
            local DropElementFrame = nil -- compat
            ...
            local function rebuildItemsInto(parentScroll)
                ...
                for idx, Item in ipairs(items) do
                    ...
                    local btn = VoidUI:Create("TextButton", {
                        ...
                        BackgroundTransparency = 1,
                        Text = tostring(Item),
                        TextTransparency = selected and 0 or 0.35,
                        TextSize = 12,
                        FontFace = Font.new([[rbxassetid://12187365364]], Enum.FontWeight.SemiBold),
                    ...
                    btn.MouseButton1Click:Connect(function()
                        ...
                        if not Dropdown.Multi then
                            ...
                            for _, r in ipairs(parentScroll:GetChildren()) do
                                if r:IsA("Frame") then
                                    local b = r:FindFirstChildOfClass("TextButton")
                                    if b then
                                        local isSel = false
                                        for _, ov in ipairs(Dropdown.Options) do
                                            if ov == b.Text then isSel = true break end
                                        end
                                        r.BackgroundTransparency = isSel and 0 or 0.55
                                        b.TextTransparency = isSel and 0 or 0.35
                                    end
                                end
                            end
            ...
            local function openDropdown()
                ...
                local menuW = math.max(absSize.X + 40, 180)
                local menuH = math.min(math.max(#(Dropdown.Option or {}) * 30 + 44, 80), 220)

                FloatingMenu = VoidUI:Create("Frame", {
                    Parent = UIScreen,
                    Name = "VoidDropdownMenu",
                    Size = UDim2.new(0, menuW, 0, 0),
                    Position = UDim2.new(0, absPos.X + absSize.X - menuW, 0, absPos.Y + absSize.Y + 6),
                ...
                -- Search (mantém pesquisa do Void Hub)
                ...
                Utility:TweenObject(FloatingMenu, {Size = UDim2.new(0, menuW, 0, menuH)}, 0.18, Enum.EasingStyle.Quint, Enum.EasingDirection.Out)
            ...
            -- compat: pesquisa no TextBox do valor (Void Hub)
            ...
            DropOptionBox:GetPropertyChangedSignal("Text"):Connect(function()
                if not isUserTyping or not DropOpen or not ScrollingFrame then return end
                local q = string.lower(DropOptionBox.Text or "")
                for _, v in ipairs(ScrollingFrame:GetChildren()) do
                    if v:IsA("Frame") then
                        local btn = v:FindFirstChildOfClass("TextButton")
                        if q == "" then
            ...
            function closeDropdown()
                ...
                if FloatingMenu then
                    ...
                    Utility:TweenObject(fm, {Size = UDim2.new(0, fm.Size.X.Offset, 0, 0)}, 0.15, Enum.EasingStyle.Quad, Enum.EasingDirection.InOut)
            ...
            UserInputService.InputBegan:Connect(function(input, gp)
                ...
                if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
                    task.defer(function()
                        if not DropOpen or not FloatingMenu then return end
                        local pos = input.Position
                        local ap = FloatingMenu.AbsolutePosition
                        local as = FloatingMenu.AbsoluteSize
                        local insideMenu = pos.X >= ap.X and pos.X <= ap.X + as.X and pos.Y >= ap.Y and pos.Y <= ap.Y + as.Y
                        local vp = DropValueFrame.AbsolutePosition
                        local vs = DropValueFrame.AbsoluteSize
                        local insideValue = pos.X >= vp.X and pos.X <= vp.X + vs.X and pos.Y >= vp.Y and pos.Y <= vp.Y + vs.Y
                        if not insideMenu and not insideValue then
                            closeDropdown()
                        end
                    end)
                end
        ...
        function Tab:Colorpicker(Config)
            ...
            local function OpenColorDialog()
                ...
                CPDragBar.MouseEnter:Connect(function()
                    Utility:TweenObject(CPDragBar, {BackgroundTransparency = 0.35}, 0.1)
                end)
                CPDragBar.MouseLeave:Connect(function()
                    Utility:TweenObject(CPDragBar, {BackgroundTransparency = 0.8}, 0.15)
                end)
        ...
        function Tab:ProgressBar(Config)
            ...
            local Beeee, Card, Inner = Utility:Element(RightScroll, ElementFrame, 48, "ProgressBar")
            ...
            local Fill = VoidUI:Create("Frame", {
                Parent = Track,
                Size = UDim2.new(math.clamp(PB.Value / PB.Max, 0, 1), 0, 1, 0),
            ...
            local ValueLabel = VoidUI:Create("TextLabel", {
                ...
                BackgroundTransparency = 1,
                Text = PB.ShowValue and (tostring(math.floor(PB.Value)) .. "%") or "",
                TextSize = 11,
                FontFace = Font.new([[rbxassetid://12187365364]], Enum.FontWeight.SemiBold),
                TextColor3 = UI.Theme.Text,
                TextXAlignment = Enum.TextXAlignment.Right,
            ...
            function PB:Set(v)
                ...
                Utility:TweenObject(Fill, { Size = UDim2.new(PB.Value / PB.Max, 0, 1, 0) }, 0.2)
                if PB.ShowValue then
                    ValueLabel.Text = tostring(math.floor(PB.Value / PB.Max * 100)) .. "%"
                end
            end
        ...
        function Tab:Code(Config)
            ...
            local Beeee, Card, Inner = Utility:Element(RightScroll, ElementFrame, 80, "Code")
    ...
    function Window:Close()
        ...
        -- Mostra o botão open arrastável (estilo WindUI / Main.lua)
    ...
    function Window:SetBackgroundVideo(videoId, transparency)
        ...
        -- VideoFrame fallback: usa ImageLabel se VideoFrame não disponível no contexto
        BackgroundImage.Image = id
        local t = transparency
        if t == nil then t = Window.BackgroundImageTransparency or 0.4 end
...
function UI:Notification(Config)
    coroutine.wrap(function()
        ...
        -- Barra de duração (estilo Main.lua)
        ...
        local DurBar = VoidUI:Create("Frame", {
            ...
            VoidUI:Create("UICorner", { CornerRadius = UDim.new(0, 2) }),
        })

        task.wait()
        local h = math.max(Card.AbsoluteSize.Y, 48)
