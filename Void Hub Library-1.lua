...
function UI:CreateWindow(Config)
    ...
    -- Posição estável em qualquer resolução/DPI (topo central, scale-based)
    local function getSafeOpenButtonPos()
        return UDim2.new(0.5, 0, 0.06, 0)
    end
    ...
    function Window:Close()
        ...
        -- mantém posição se o usuário arrastou; senão usa topo central estável (DPI-safe)
        if not Window._OpenBtnUserMoved then
