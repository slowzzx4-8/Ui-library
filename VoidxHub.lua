...
function Utility:Element(RightScroll, ElementFrame, sizeY, scope)
    local contentW = ElementFrame.Size.X.Offset
    if contentW <= 0 then
        contentW = math.max(120, (ElementFrame.AbsoluteSize and ElementFrame.AbsoluteSize.X or 0) - 10)
    else
        contentW = contentW - 10
    end
    if contentW <= 0 then
        -- fallback: fill parent scroll width
        contentW = nil
    end
    local Beeee = VoidUI:Create("Frame", {
        ...
        AutomaticSize = "Y",
        Size = contentW and UDim2.new(0, contentW, 0, sizeY or 44) or UDim2.new(1, -10, 0, sizeY or 44),
        ZIndex = 15,
    })
...
function UI:CreateWindow(Config)
    local Window = {
        Name = Config.Name or "VoidxHub",
        Author = Config.Author or "By Slowzzx",
        Icon = Config.Icon or "house",
    ...
    local function getDefaultOpenButtonPos()
        -- lado esquerdo, mais PARA CIMA (não no centro)
        local cam = workspace.CurrentCamera
        local vp = (cam and cam.ViewportSize) or Vector2.new(1920, 1080)
        local x = OPEN_BTN_MARGIN + 4
        local y = math.floor(math.clamp(vp.Y * 0.12, 48, 120))
    ...
    local Main = VoidUI:Create("Frame", {
        ...
        ClipsDescendants = false, -- resize handle fica FORA da UI
        Active = true,
    ...
    local ElementFolder = VoidUI:Create("Frame", {
        Name = "ElementFolder",
        Parent = Main,
        BackgroundTransparency = 1,
        BorderSizePixel = 0,
        ClipsDescendants = true,
        Position = UDim2.new(0, Window.SideBarWidth + 4, 0, Window.Topbar.Height + 13),
        Size = UDim2.new(1, -(Window.SideBarWidth + 12), 1, -(Window.Topbar.Height + 20)),
        ZIndex = 4,
    })
    ...
    function Window:Tab(Config, type)
        ...
        local ElementFrame = VoidUI:Create("Frame", {
            ...
            Name = "ElementFrame_" .. tostring(#Window.TabList + 1),
            AnchorPoint = Vector2.new(0, 0),
            Position = UDim2.new(0, 0, 0, 0),
            BorderColor3 = Color3.new(0, 0, 0),
            ClipsDescendants = true,
        ...
        -- Empty state ("Not found") when tab has no elements
        local EmptyState = VoidUI:Create("Frame", {
            Parent = ElementFrame,
            Name = "EmptyState",
            BackgroundTransparency = 1,
            Size = UDim2.new(1, 0, 1, 0),
        ...
        local EmptyIcon = VoidUI:Create("ImageLabel", {
            Parent = EmptyState,
            BackgroundTransparency = 1,
            Size = UDim2.new(0, 42, 0, 42),
            Image = ResolveIconImage("frown"),
        ...
        do
            local d = GetIcon("frown")
            if typeof(d) == "table" then
                EmptyIcon.Image = d.Image or EmptyIcon.Image
                if d.ImageRectOffset then EmptyIcon.ImageRectOffset = d.ImageRectOffset end
                if d.ImageRectSize then EmptyIcon.ImageRectSize = d.ImageRectSize end
            elseif typeof(d) == "string" then
                EmptyIcon.Image = d
            end
        end
        ...
        local function updateEmptyState()
            ...
            RightScroll.Visible = hasContent
        end
        RightScroll.ChildAdded:Connect(function()
            task.defer(updateEmptyState)
        end)
        RightScroll.ChildRemoved:Connect(function()
            task.defer(updateEmptyState)
        end)
        task.defer(updateEmptyState)
       ...
       local function SelectTab()
            UI:CloseActiveDropdown()
            -- SAVE index immediately (before any UI work) so minimize never loses it
            local myIndex = nil
            for i, t in ipairs(Window.TabList or {}) do
                if t == Tab then
                    myIndex = i
                    break
                end
            ...
            if myIndex then
                Window.SelectedTabIndex = myIndex
                Window._LastTabIndex = myIndex
            end
            for _, v in ipairs(ElementFolder:GetChildren()) do
                if v:IsA("GuiObject") then
                    v.Visible = false
                end
            end
            ElementFrame.Visible = true
            ElementFrame.Size = UDim2.new(1, 0, 1, Tags > 0 and -37 or 0)
        ...
        -- Select first tab only once (initial). After that keep last player choice.
        if #Window.TabList == 1 and not Window._HasInitialTab then
            Window._HasInitialTab = true
        ...
        function Tab:Paragraph(Config,type)
            ...
            local ParagraphFrame = VoidUI:Create("Frame", {
                ...
                ClipsDescendants = true,
                BackgroundTransparency = 0.5,
                Size = UDim2.new(1, -10, 0, Paragraph.SizeY),
        ...
        function Tab:Toggle(Config,type)
            ...
            function Togglee:SetValue(newValue)
                ...
                local trackOn = Color3.fromRGB(255, 255, 255) -- branco ativo
                if newValue then
        ...
        function Tab:Slider(Config)
            local valueTbl = Config.Value
            if type(valueTbl) ~= "table" then
                valueTbl = {
                    Min = Config.Min or 0,
        ...
        function Tab:Dropdown(Config)
            local Dropdown = {
                ...
                Desc = Config.Desc,
                Value = Config.Value, -- nil = nothing selected (no bug)
                Locked = Config.Locked or false,
                ...
                Locked = Config.Locked,
                Callback = Config.Callback or function() end,
                ASpeed = Config.ASpeed or 0.2
            }

            -- Normalize values: nothing selected by default
            if Dropdown.Multi then
                if type(Dropdown.Value) == "table" then
                    Dropdown.Options = {}
                    for _, v in ipairs(Dropdown.Value) do table.insert(Dropdown.Options, v) end
                elseif Dropdown.Value and Dropdown.Value ~= "" then
            ...
            local DropOptionBox = VoidUI:Create("TextBox", {
                ...
                Text = (function()
                    if Dropdown.Multi then
                        return (#Dropdown.Options > 0) and table.concat(Dropdown.Options, ", ") or ""
                    end
                    return (Dropdown.Value ~= nil and Dropdown.Value ~= "") and tostring(Dropdown.Value) or ""
                end)(),
            ...
            local DropElementFrame = VoidUI:Create("Frame", {
                Parent = DropDownElement,
                BackgroundTransparency = 1,
                ClipsDescendants = true,
                Size = UDim2.new(1, 0, 0, 0),
            ...
            local ScrollingFrame = VoidUI:Create("ScrollingFrame", {
            ...
            local ListLayout = VoidUI:Create("UIListLayout", {
            ...
            function Dropdown:Refresh(options)
                ...
                for _, Item in ipairs(Dropdown.Option) do
                    local DropElement = VoidUI:Create("Frame", {
                        Parent = ScrollingFrame,
                        Name = Item,
                        BackgroundColor3 = Color3.fromRGB(43, 43, 43),
                        BackgroundTransparency = 0.5,
                    ...
                    local DropElementTRG = VoidUI:Create("TextButton", {
                        Parent = DropElement,
                        BackgroundTransparency = 1,
                        Size = UDim2.new(1, 0, 1, 0),
                        Text = Item,
                        ...
                        VoidUI:Create("UIPadding", { PaddingLeft = UDim.new(0, 8) })
                    })
            ...
            if registerElement then registerElement(Dropdown) end
            return Dropdown
        end
        ...
        function Tab:Colorpicker(Config)
            ...
            local function OpenColorDialog()
                ...
                -- Código HEX editável (abaixo do preview)
                local HexInputFrame = VoidUI:Create("Frame", {
                    Parent = Dialog,
                    Size = UDim2.new(0, 54, 0, 22),
                    Position = UDim2.new(0, 258, 0, 110),
                    BackgroundTransparency = 0.3,
                    BackgroundColor3 = UI.Theme.ElementColor or Color3.fromRGB(30, 30, 34),
                    ZIndex = 302,
                    ThemeID = { BackgroundColor3 = "ElementColor" },
                }, {
                ...
                local HexLabel = VoidUI:Create("TextBox", {
                ...
                local function parseHexToColor(str)
                ...
                local function UpdateDisplay(skipHexText)
                    ...
                    if not skipHexText and not HexLabel:IsFocused() then
                        HexLabel.Text = "#" .. col:ToHex()
                    end
                end

                local function applyHexFromInput()
                    local col = parseHexToColor(HexLabel.Text)
                    if not col then
                    ...
                    if Colorpicker.Callback then
                        task.spawn(Colorpicker.Callback, col)
                    end
                end

                HexLabel.FocusLost:Connect(function(enter)
                    applyHexFromInput()
                end)
                HexLabel:GetPropertyChangedSignal("Text"):Connect(function()
                    if not HexLabel:IsFocused() then return end
                    local col = parseHexToColor(HexLabel.Text)
                    if col then
                        h, s, v = Color3.toHSV(col)
                        UpdateDisplay(true)
        ...
        function Tab:SupportStatus(Config)
            Config = Config or {}
            Config.Parent = RightScroll
            Config.AsElement = true
            return Window:SupportStatus(Config)
        end
    ...
    function Window:Open()
        ...
        -- restaura a última aba (nunca força a 1 se o player estava em outra)
        local idx = tonumber(Window._LastTabIndex) or tonumber(Window.SelectedTabIndex)
        if not idx or idx < 1 or not (Window.TabList and Window.TabList[idx]) then
            idx = 1
        end
        Window.SelectedTabIndex = idx
        Window._LastTabIndex = idx
        pcall(function()
            local tab = Window.TabList[idx]
            if tab and tab.Select then
                tab.Select()
            else
    ...
    function Window:Close()
        ...
        if Window.SelectedTabIndex then
            Window._LastTabIndex = Window.SelectedTabIndex
        end
    ...
    function Window:Resize(sizeX, sizeY, isDrag)
        ...
        -- Keep ElementFolder geometry in sync with window size
        if ElementFolder and ElementFolder:IsA("GuiObject") then
            ElementFolder.Position = UDim2.new(0, Window.SideBarWidth + 4, 0, Window.Topbar.Height + 13)
            ElementFolder.Size = UDim2.new(1, -(Window.SideBarWidth + 12), 1, -(Window.Topbar.Height + 20 + (Tags > 0 and 37 or 0)))
        end
        for _, EF in ipairs(ElementFolder:GetChildren()) do
            if EF:IsA("Frame") and EF.Name:find("ElementFrame") then
                ...
                if Scroll then
                    for _, item in ipairs(Scroll:GetChildren()) do
                        if item:IsA("Frame") then
                            local w = math.max(80, sizeX - Window.SideBarWidth - 18)
                            item.Size = UDim2.new(0, w, 0, item.Size.Y.Offset)
                        end
                    end
                end
            end
    ...
    -- Resize handle FORA da UI (parent = UIScreen), igual à linha de arrastar / VexUI
    ...
    local function UpdateResizeHandlePos()
        if not Main or not Main.Parent then return end
        -- FORA da borda da UI (canto inferior direito, ligeiramente externo)
    ...
    UserInputService.InputChanged:Connect(function(input)
        if resizing and (input.UserInputType == Enum.UserInputType.MouseMovement or input.UserInputType == Enum.UserInputType.Touch) then
            ...
            -- Main usa AnchorPoint (0.5, 0.5): delta * 2 para crescer do centro
    ...
    function Window:SetResizable(v)
        ResizeHandle.Visible = v and true or false
    ...
    -- Support status (real-time icon/image + text)
    -- Usage: local s = Window:SupportStatus({ Title = "Support", Status = "Online", Icon = "check-circle" })
    --        s:SetStatus("Busy", "clock")  s:SetImage("rbxassetid://...")  s:SetDesc("...")
    function Window:SupportStatus(Config)
        Config = Config or {}
        local statuses = {
            Online  = { Color = Color3.fromRGB(34, 197, 94),  Icon = "check-circle" },
            Offline = { Color = Color3.fromRGB(148, 163, 184), Icon = "x-circle" },
            Busy    = { Color = Color3.fromRGB(249, 115, 22), Icon = "clock" },
            Away    = { Color = Color3.fromRGB(234, 179, 8),  Icon = "moon" },
            Maintenance = { Color = Color3.fromRGB(239, 68, 68), Icon = "wrench" },
            Connecting  = { Color = Color3.fromRGB(59, 130, 246), Icon = "loader" },
        }
        -- custom colors from Config.Colors = { Online = Color3..., MyStatus = Color3... }
        if type(Config.Colors) == "table" then
            for name, col in pairs(Config.Colors) do
                if typeof(col) == "Color3" then
                    statuses[name] = statuses[name] or { Icon = "circle" }
                    statuses[name].Color = col
                elseif type(col) == "table" then
                    statuses[name] = {
                        Color = col.Color or Color3.fromRGB(255, 255, 255),
                        Icon = col.Icon or (statuses[name] and statuses[name].Icon) or "circle",
                    }
                end
            end
        end
        ...
        local state = {
            Title = Config.Title or "Support",
            Status = Config.Status or "Online",
            Desc = Config.Desc or Config.Content or "",
            Icon = Config.Icon,
            Image = Config.Image, -- rbxassetid or url
            Parent = Config.Parent, -- optional ElementFrame/RightScroll; default Notification area card in window
        }

        local holder = Config.Parent
        if not holder then
            -- floating status card bottom-left of Main
            holder = VoidUI:Create("Frame", {
                Parent = Main,
                Name = "SupportStatusHolder",
                AnchorPoint = Vector2.new(0, 1),
        ...
        local card = holder
        if holder == Config.Parent and Config.AsElement then
            -- embedded in tab content
            card = VoidUI:Create("Frame", {
                Parent = holder,
        ...
        local avatar = VoidUI:Create("ImageLabel", {
            Parent = card,
            Size = UDim2.new(0, 36, 0, 36),
            Position = UDim2.new(0, 10, 0.5, 0),
            AnchorPoint = Vector2.new(0, 0.5),
        ...
        local iconImg = VoidUI:Create("ImageLabel", {
            ...
            BackgroundTransparency = 1,
            Image = ResolveIconImage("check-circle"),
            ZIndex = 252,
            ThemeID = { ImageColor3 = "IconColor" },
        })

        local titleLbl = VoidUI:Create("TextLabel", {
            Parent = card,
            BackgroundTransparency = 1,
        ...
        local statusLbl = VoidUI:Create("TextLabel", {
            Parent = card,
            BackgroundTransparency = 1,
            Position = UDim2.new(0, 74, 0, 28),
        ...
        local descLbl = VoidUI:Create("TextLabel", {
            Parent = card,
            BackgroundTransparency = 1,
            ...
            TextSize = 11,
            TextTransparency = 0.4,
            Visible = state.Desc ~= "" and state.Desc ~= nil,
            FontFace = Font.new([[rbxassetid://12187365364]], Enum.FontWeight.Medium),
            TextXAlignment = Enum.TextXAlignment.Left,
            TextWrapped = true,
            ZIndex = 252,
        ...
        local function applyIcon(name)
            local d = GetIcon(name or "circle")
            if typeof(d) == "table" then
                iconImg.Image = d.Image or ""
                if d.ImageRectOffset then iconImg.ImageRectOffset = d.ImageRectOffset end
                if d.ImageRectSize then iconImg.ImageRectSize = d.ImageRectSize end
            elseif typeof(d) == "string" then
                iconImg.Image = d
            else
                iconImg.Image = ResolveIconImage(name or "circle")
            end
        end

        function api:SetStatus(statusName, iconOverride, colorOverride)
            -- SetStatus("Busy") or SetStatus("Busy", "clock") or SetStatus("Busy", "clock", Color3...)
            -- or SetStatus({ Status="Busy", Icon="clock", Color=Color3..., Desc="..." })
            if type(statusName) == "table" then
                local t = statusName
                statusName = t.Status or t.Name or state.Status
                iconOverride = t.Icon or iconOverride
                colorOverride = t.Color or colorOverride
                if t.Desc or t.Content then api:SetDesc(t.Desc or t.Content) end
                if t.Title then api:SetTitle(t.Title) end
                if t.Image then api:SetImage(t.Image) end
            end
            state.Status = tostring(statusName or state.Status)
            statusLbl.Text = state.Status
            local meta = statuses[state.Status]
            local col = colorOverride
            if not col and meta then col = meta.Color end
            if typeof(col) == "Color3" then
                dot.BackgroundColor3 = col
                -- remember custom color for this status name
                statuses[state.Status] = statuses[state.Status] or { Icon = "circle" }
                statuses[state.Status].Color = col
            elseif meta then
                dot.BackgroundColor3 = meta.Color
            end
            applyIcon(iconOverride or state.Icon or (meta and meta.Icon) or "circle")
            return api
        end

        function api:SetColor(statusName, color)
            if typeof(statusName) == "Color3" and color == nil then
                -- SetColor(Color3) applies to current status
                color = statusName
                statusName = state.Status
            end
            if typeof(color) == "Color3" then
                statuses[tostring(statusName)] = statuses[tostring(statusName)] or { Icon = "circle" }
                statuses[tostring(statusName)].Color = color
                if tostring(statusName) == state.Status then
        ...
        function api:SetColors(map)
            if type(map) ~= "table" then return api end
            for name, col in pairs(map) do
                if typeof(col) == "Color3" then
                    api:SetColor(name, col)
                elseif type(col) == "table" and typeof(col.Color) == "Color3" then
                    statuses[name] = { Color = col.Color, Icon = col.Icon or "circle" }
                    if name == state.Status then
                        dot.BackgroundColor3 = col.Color
                        if col.Icon then applyIcon(col.Icon) end
                    end
                end
            end
            return api
        ...
        function api:SetTitle(t)
            state.Title = tostring(t or "")
            titleLbl.Text = state.Title
            return api
        end

        function api:SetDesc(d)
            state.Desc = d and tostring(d) or ""
            descLbl.Text = state.Desc
            descLbl.Visible = state.Desc ~= ""
            return api
        end
        ...
        function api:SetVisible(v)
            card.Visible = v and true or false
            return api
        end
        ...
        -- real-time poll helper (optional)
        function api:Bind(fetchFn, interval)
            interval = interval or 5
            if api._bindThread then
                task.cancel(api._bindThread)
                api._bindThread = nil
            end
            api._bindThread = task.spawn(function()
                while card and card.Parent do
                    local ok, result = pcall(fetchFn)
                    if ok and result then
                        if type(result) == "string" then
                            api:SetStatus(result)
                        elseif type(result) == "table" then
                            if result.Status then api:SetStatus(result.Status, result.Icon) end
                            if result.Title then api:SetTitle(result.Title) end
                            if result.Desc or result.Content then api:SetDesc(result.Desc or result.Content) end
                            if result.Image then api:SetImage(result.Image) end
                            if result.Icon and not result.Status then api:SetIcon(result.Icon) end
                        end
                    end
                    task.wait(interval)
                end
        ...
        -- initial paint
        api:SetStatus(state.Status, state.Icon)
        if state.Image then api:SetImage(state.Image) end
        return api
    end

    -- Tab helper: SupportStatus as content element
    -- (also available via Window:SupportStatus({ Parent = RightScroll, AsElement = true }))
