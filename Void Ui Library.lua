...
function UI:CreateWindow(Config)
    ...
    function Window:Tab(Config, type)
       ...
       local function SelectTab()
            UI:CloseActiveDropdown()
            local targetW = Window.Size.X.Offset - Window.SideBarWidth - 8
            ...
            -- sem animação de expandir (removido tween de Size do scroll/content)
            ElementFrame.Size = UDim2.new(0, targetW, 0, targetH)
            -- flash estilo botões da UI
            ElementFrame.BackgroundTransparency = 0
            Utility:TweenObject(ElementFrame, {BackgroundTransparency = 0.2}, 0.1, Enum.EasingStyle.Linear, Enum.EasingDirection.Out)
            for _, v in next, Window.Tabs do
                Utility:TweenObject(v, {BackgroundTransparency = 1}, 0.1, Enum.EasingStyle.Linear, Enum.EasingDirection.Out)
                for _, obj in ipairs(v:GetChildren()) do
                    if obj:IsA("TextLabel") then
                        Utility:TweenObject(obj, {TextTransparency = 0.5}, 0.1, Enum.EasingStyle.Linear, Enum.EasingDirection.Out)
                    elseif obj:IsA("UIStroke") then
                        Utility:TweenObject(obj, {Transparency = 1}, 0.1, Enum.EasingStyle.Linear, Enum.EasingDirection.Out)
                    elseif obj:IsA("ImageLabel") then
                        Utility:TweenObject(obj, {ImageTransparency = 0.5}, 0.1, Enum.EasingStyle.Linear, Enum.EasingDirection.Out)
                    end
                end
            end
            -- animação de clique nos botões da sidebar
            TabBack.BackgroundTransparency = 0
            Utility:TweenObject(TabBack, {BackgroundTransparency = (Tab.Border and 0.6 or 1)}, 0.1, Enum.EasingStyle.Linear, Enum.EasingDirection.Out)
            Utility:TweenObject(TabBack.UIStroke, {Transparency = (Tab.Border and 0 or 1)}, 0.1, Enum.EasingStyle.Linear, Enum.EasingDirection.Out)
            Utility:TweenObject(TabTitle, {TextTransparency = 0}, 0.1, Enum.EasingStyle.Linear, Enum.EasingDirection.Out)
            if TabIcon then
                Utility:TweenObject(TabIcon, {ImageTransparency = 0}, 0.1, Enum.EasingStyle.Linear, Enum.EasingDirection.Out)
            end
            -- mantém a aba escolhida ao minimizar/abrir de novo
            for idx, t in ipairs(Window.TabList) do
                if t == Tab then
        ...
        function Tab:Paragraph(Config,type)
            ...
            if Paragraph.Icon then
                local pIconData = GetIcon(Paragraph.Icon)
                local pIconImg = ResolveIconImage(Paragraph.Icon)
                Icon = VoidUI:Create("ImageLabel", {
                    AnchorPoint = Vector2.new(0, 0.5),
                    Image = pIconImg,
                    BackgroundTransparency = 1,
                ...
                if typeof(pIconData) == "table" then
                    if pIconData.ImageRectOffset then Icon.ImageRectOffset = pIconData.ImageRectOffset end
                    if pIconData.ImageRectSize then Icon.ImageRectSize = pIconData.ImageRectSize end
                end
            ...
            function Paragraph:SetContent(Text)
                if Desc and Desc.SetText then
                    Desc.SetText(tostring(Text or ""))
                    if Desc.Frame then Desc.Frame.Visible = true end
                end
            end
        ...
        function Tab:Slider(Config)
            ...
            local DropValue = VoidUI:Create("Frame", {
                ...
                ZIndex = 16,
                ThemeID = {
                    BackgroundColor3 = "Slider.SliderPart|IconColor|Outline"
                }
            }, {
                VoidUI:Create("UICorner", {
                    CornerRadius = UDim.new(0, 12),
        ...
        function Tab:Dropdown(Config)
                ...
                for idx, Item in ipairs(items) do
                            ...
                            if r:IsA("TextButton") then
                                ...
                                if lab then
                                    ...
                                    if Dropdown.Multi then
                                        ...
                                        isSel = (Dropdown.Value ~= nil and tostring(Dropdown.Value) == lab.Text)
                                    end
                                    -- selecionado fica bem mais claro; desmarcado volta ao normal
                                    r.BackgroundTransparency = isSel and 0.05 or 1
                                    lab.TextTransparency = isSel and 0 or 0.35
                                end
                            end
                        end
                    end
                    ...
                    local function pick()
                        ...
                        if not Dropdown.Multi then
                            -- clicar de novo no mesmo item = desmarcar
                            if Dropdown.Value == Item then
                                Dropdown.Value = nil
                                DropOptionBox.Text = ""
                                refreshRowHighlights()
                                task.spawn(Dropdown.Callback, nil)
                            else
                                Dropdown.Value = Item
            ...
            local function placeMenuOutsideRight(menu, menuW, menuH)
                if not menu or not Main then return end
                local mainPos = Main.AbsolutePosition
                local mainSize = Main.AbsoluteSize
                -- altura total da UI (topo até baixo), mesmo com poucas opções
                local h = math.max(mainSize.Y, 120)
                local w = math.max(menuW or 180, 140)
                -- fora da UI, lado direito, encostado na borda
                local gap = 2
            ...
            local function openDropdown()
                ...
                task.defer(function()
                    ...
                    -- altura sempre do topo ao fundo da UI (mesmo com 2 opções)
                    local menuH = math.max(mainSize.Y, 120)
                    menuW = math.clamp(menuW, 160, 220)
                    ...
                    placeMenuOutsideRight(FloatingMenu, menuW, menuH)
            ...
            function Dropdown:SetValues(options)
                return Dropdown:Refresh(options)
            end
    ...
    function Window:Notify(cfg)
        ...
        UI:Notification({
            ...
            Desc = cfg.Desc or cfg.Content or nil,
            Icon = cfg.Icon,
            Duration = cfg.Duration or 5,
            CanClose = cfg.CanClose,
        })
        return Window
    end
