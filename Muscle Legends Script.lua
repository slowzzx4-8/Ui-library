...
local function findRockByDurability(dur)
    if rockCache[dur] and rockCache[dur].Parent then
        return rockCache[dur]
    end
...
local function setFastPunch(enabled)
    ...
    spawnTask("fastPunchEquip", function()
        while State.running and State.fastPunch do
            pcall(function()
    ...
    spawnTask("fastPunchHit", function()
        while State.running and State.fastPunch do
            ...
            if punch then pcall(punch.Activate, punch) end
            task.wait(0.01)
        end
    end)
...
local function useMachine(machine)
    ...
    local folder = workspace:FindFirstChild("machinesFolder")
    local obj = folder and folder:FindFirstChild(machine.object)
    local seat = obj and obj:FindFirstChild("interactSeat", true)
    if machine.object == "Jungle Squat" and seat and seat:IsA("BasePart") then
        hrp.CFrame = seat.CFrame * CFrame.new(0, 2.25, 0)
    else
...
local function setFastSpeed(enabled)
    ...
    if State.fastSpeed or was then updateWalkSpeed() end
    if was and not State.fastSpeed then savedWalkSpeed = nil end
end
...
local Window = VoidUI:CreateWindow({
    Name = "Muscle Legends",
    Icon = "rbxassetid://93173535299359",
    SideBarWidth = 160,
    Theme = "Red",
    Transparent = true,
    Author = "By Slowzzx4",
...
-- reforça ícones / outline vermelhos
pcall(function()
    Window:SetTheme("Red")
end)
...
-- Watermark = nome do player + ping (ao vivo)
local function updateWatermark()
    local name = LocalPlayer.DisplayName or LocalPlayer.Name or "Player"
    local ping = 0
    pcall(function() ping = getPing() end)
    pcall(function()
        Window:SetWatermark(string.format("%s  •  %d ms", name, ping))
    end)
end
updateWatermark()
spawnTask("watermarkPing", function()
...
Tabs.Main:Slider({
    ...
    Callback = function(v)
        ...
        local remote = events and events:FindFirstChild("changeSpeedSizeRemote")
        if remote then
            pcall(remote.InvokeServer, remote, "changeSize", v)
        end
    end,
})
Tabs.Main:Toggle({
        ...
        if v then
            spawnTask("mainAutoSize", function()
                while State.running and State.mainAutoSize do
                    ...
                    local remote = events and events:FindFirstChild("changeSpeedSizeRemote")
                    if remote then
                        pcall(remote.InvokeServer, remote, "changeSize", State.mainSize)
                    end
                    task.wait(1)
                end
            end)
...
Tabs.Main:Slider({
    Title = "Speed",
    Value = { Min = 16, Max = 500, Default = 125 },
    Step = 1,
    Callback = function(v)
        State.mainSpeed = v
        local events = getREvents()
        local remote = events and events:FindFirstChild("changeSpeedSizeRemote")
        if remote then
            pcall(remote.InvokeServer, remote, "changeSpeed", v)
        end
    end,
})
Tabs.Main:Toggle({
        ...
        if v then
            spawnTask("mainAutoSpeed", function()
                while State.running and State.mainAutoSpeed do
                    ...
                    local remote = events and events:FindFirstChild("changeSpeedSizeRemote")
                    if remote then
                        pcall(remote.InvokeServer, remote, "changeSpeed", State.mainSpeed)
                    end
                    task.wait(1)
                end
            end)
...
Tabs.Main:Toggle({
    Title = "Fast Speed",
    Default = false,
    Callback = function(v) setFastSpeed(v) end,
})
Tabs.Main:Toggle({
    Title = "Walk on Water",
    Default = false,
    Callback = function(v) setWalkWater(v) end,
})
Tabs.Main:Toggle({
    Title = "Noclip",
    Default = false,
...
Tabs.Main:Toggle({
    ...
    Default = false,
    Callback = function(v) State.setAntiKnockback(v) end,
})

-- ========== GLITCH ==========
Tabs.Glitch:TabSection({ Title = "Fast Punch" })
local rockToggles = {}
Tabs.Glitch:Toggle({
    Title = "Fast Punch",
    Default = false,
    Callback = function(v)
        setFastPunch(v)
        for _, t in ipairs(rockToggles) do
            -- lock/unlock handled by selection
        end
        if not v then clearRockSession() end
    end,
})
Tabs.Glitch:Toggle({
    Title = "Hide Durability",
    Default = false,
    Callback = function(v) setHideDurability(v) end,
})
...
for _, rock in ipairs(Config.Rocks) do
    ...
    Tabs.Glitch:Toggle({
        ...
        Callback = function(v)
            if v then
                if not State.fastPunch then
                    Window:Notify({ Title = "Glitch", Content = "Enable Fast Punch first", Duration = 2 })
                    return false
                end
                clearRockSession()
                State.selectedRock = r
...
Tabs.AutoFarm:Button({
    ...
    Callback = function()
        ...
        if ok then
            Window:Notify({ Title = "Gamepass", Content = "Auto Lift unlocked on client", Duration = 2 })
        else
            Window:Notify({ Title = "Gamepass", Content = "Could not unlock", Duration = 2 })
        end
    end,
})
...
Tabs.Rebirths:Toggle({
    ...
    Callback = function(v)
        if v and not State.rebirth.target then
            Window:Notify({ Title = "Rebirths", Desc = "Set objective first", Duration = 2 })
            return false
        end
...
Tabs.Gifts:Button({
    ...
    Callback = function()
        if not giftPlayer then
            Window:Notify({ Title = "Gifts", Content = "Choose a player", Duration = 2 })
            return
        end
...
Tabs.Trade:Dropdown({
    ...
    Value = "6",
    Callback = function(v)
        tradeAmount = math.clamp(tonumber(v) or 6, 1, 6)
    end,
})
Tabs.Trade:Toggle({
    ...
    Callback = function(v)
        ...
        if not tradePlayer or not tradePet then
            Window:Notify({ Title = "Trade", Content = "Select player and pet", Duration = 2 })
            return false
        end
        local rEvents = ReplicatedStorage:FindFirstChild("rEvents")
        local remote = rEvents and (rEvents:FindFirstChild("tradingEvent") or rEvents:FindFirstChild("tradeRemote"))
        if not remote then
            Window:Notify({ Title = "Trade", Content = "Trade remote not found", Duration = 2 })
            return false
        end
...
local sessionParagraph = Tabs.Stats:Paragraph({
    Title = "Session",
    Desc = "0d 0h 0m 0s",
})
local strengthParagraph = Tabs.Stats:Paragraph({ Title = "Strength", Desc = "0 (+0)" })
local durabilityParagraph = Tabs.Stats:Paragraph({ Title = "Durability", Desc = "0 (+0)" })
local rebirthsParagraph = Tabs.Stats:Paragraph({ Title = "Rebirths", Desc = "0 (+0)" })
local killsParagraph = Tabs.Stats:Paragraph({ Title = "Kills", Desc = "0 (+0)" })
local evilParagraph = Tabs.Stats:Paragraph({ Title = "Evil Karma", Desc = "0 (+0)" })
local goodParagraph = Tabs.Stats:Paragraph({ Title = "Good Karma", Desc = "0 (+0)" })
local pingParagraph = Tabs.Stats:Paragraph({ Title = "Ping", Desc = "0 ms" })

local paraMap = {
    Strength = strengthParagraph,
    Durability = durabilityParagraph,
    Rebirths = rebirthsParagraph,
    Kills = killsParagraph,
    ["Evil Karma"] = evilParagraph,
    ["Good Karma"] = goodParagraph,
}

local function setParagraphContent(para, text)
    if not para then return end
    pcall(function()
        if para.SetContent then
            para:SetContent(text)
        elseif para.Set then
            para:Set(text)
        elseif para.Content ~= nil then
            para.Content = text
        elseif typeof(para) == "Instance" then
            local label = para:FindFirstChildWhichIsA("TextLabel", true)
...
local function activeStatusText()
    ...
    if State.kill.auto then parts[#parts + 1] = "Kill Everyone" end
    ...
    if State.fastSpeed then parts[#parts + 1] = "Fast Speed" end
...
spawnTask("statsUpdater", function()
    while State.running do
        ...
        setParagraphContent(sessionParagraph, string.format("%dd %dh %dm %ds",
            math.floor(elapsed / 86400),
            math.floor((elapsed % 86400) / 3600),
            math.floor((elapsed % 3600) / 60),
            elapsed % 60))
        setParagraphContent(statusParagraph, activeStatusText())
        setParagraphContent(pingParagraph, tostring(getPing()) .. " ms")
...
Tabs.Stats:Button({
    ...
    Callback = function()
        ...
        Window:Notify({ Title = "Stats", Content = "Baseline reset", Duration = 2 })
    end,
})
...
Tabs.Misc:TabSection({ Title = "Spy" })
local spyDropdown = Tabs.Misc:Dropdown({
    Title = "Choose Player",
    Option = playerList(),
    Value = nil,
    Callback = function(v)
...
Tabs.Misc:Toggle({
    ...
    Callback = function(v)
        if v and not State.spyTarget then
            Window:Notify({ Title = "Spy", Content = "Select a player first", Duration = 2 })
            return false
        end
        setSpy(v)
    end,
...
Tabs.Misc:Button({
    ...
    Callback = function()
        if bgInputText == "" then
            Window:Notify({ Title = "Background", Content = "Enter an image id first", Duration = 2 })
            return
        end
...
Tabs.Misc:Button({
    ...
    Callback = function()
        ...
        Window:Notify({ Title = "Background", Content = "Cleared", Duration = 2 })
    end,
})
Tabs.Misc:Button({
    Title = "Acrylic On",
    Callback = function()
        ...
        Window:Notify({ Title = "Acrylic", Content = "Enabled", Duration = 2 })
    end,
})
Tabs.Misc:Button({
    Title = "Reset Acrylic",
    Callback = function()
        ...
        Window:Notify({ Title = "Acrylic", Content = "Reset", Duration = 2 })
    end,
})
