...
function UI:CreateWindow(Config)
       ...
       local function SelectTab()
        ...
        function Tab:Dropdown(Config)
            ...
            local function rebuildItemsInto(parentScroll)
                ...
                for idx, Item in ipairs(items) do
                    ...
                    -- estilo lista limpa (igual referência): só texto, sem caixa de botão
                    local row = VoidUI:Create("TextButton", {
                        Parent = parentScroll,
                            ...
                            for _, r in ipairs(parentScroll:GetChildren()) do
                                if r:IsA("TextButton") then
                                    ...
                                    if lab then
                                        ...
                                        lab.TextTransparency = isSel and 0 or 0.15
                                    end
                                end
                            end
                            task.spawn(Dropdown.Callback, Dropdown.Options)
                        end
                        task.delay(0.04, function() selectingItem = false end)
                    end
            ...
            local function openDropdown()
                ...
                task.defer(function()
                    ...
                    local menuW = math.clamp(rsW - 12, 180, mainSize.X - 20)
                    local itemCount = #(Dropdown.Option or {})
                    local menuH = math.min(math.max(itemCount * 36 + 52, 120), math.min(320, mainSize.Y - 20))
                    ...
                    -- Search centralizado no topo (estilo referência)
        ...
        function Tab:SegmentedControl(Config)
            ...
            local bar = VoidUI:Create("Frame", {
                ...
                ZIndex = 16,
            }, { VoidUI:Create("UICorner", { CornerRadius = UDim.new(0, 8) }), VoidUI:Create("UIListLayout", { FillDirection = Enum.FillDirection.Horizontal, HorizontalFlex = Enum.UIFlexAlignment.Fill, Padding = UDim.new(0, 2) }) })
            local buttons = {}
            for i, opt in ipairs(SC.Options) do
                local on = (opt == SC.Value)
                local b = VoidUI:Create("TextButton", {
                    Parent = bar, Size = UDim2.new(1, 0, 1, -2), BackgroundTransparency = on and 0 or 1,
    ...
    -- Acrylic simulado (transparência forte)
    function Window:ToggleAcrylic(enabled)
        Window.Acrylic = enabled ~= false
        if Window.Acrylic then
            Utility:TweenObject(Main, { BackgroundTransparency = 0.55 }, 0.25)
        else
            Utility:TweenObject(Main, { BackgroundTransparency = Window.Transparent and 0.15 or 0 }, 0.25)
        end
        return Window
    end
