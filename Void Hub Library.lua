...
function UI:CreateWindow(Config)
    ...
    if Window.KeySystem and Window.KeySystem.KeyValidator then
        ...
        if not isfolder("VoidUI/" .. (Window.Folder or "Temp")) then makefolder("VoidUI/" .. (Window.Folder or "Temp")) end
        if isfile("VoidUI/" .. (Window.Folder or "Temp") .. "/key.json") then
            KeyFrame.Frame.TextBox.Text = game:GetService("HttpService"):JSONDecode(readfile("VoidUI/" .. (Window.Folder or "Temp") .. "/key.json")).key or ""
        end
        ...
        local Kframe1 = VoidUI:Create("Frame", {
        ...
        local Kframe2 = VoidUI:Create("Frame", {
            Parent = Kframe1,
        ...
        local Abutton = VoidUI:Create("TextButton", {
            Parent = Kframe2,
            TextSize = 12,
        ...
        local Gbutton = VoidUI:Create("TextButton", {
            Parent = Kframe2,
            TextSize = 12,
    ...
    function Window:Open()
        ...
        -- Ajuste preciso do LeftScroll para não vazar pro UserFrame
        Utility:TweenObject(LeftScroll, {Size = UDim2.new(1, 0, 1, scrollOffset)}, 0.3, Enum.EasingStyle.Back, Enum.EasingDirection.Out)
        Utility:TweenObject(Main.Frame, {Size = UDim2.new(0, Window.Size.X.Offset, 0, Window.Size.Y.Offset - 8)}, 0.3, Enum.EasingStyle.Back, Enum.EasingDirection.Out)
        ...
        if Window.Resizable and ResizeHandle then ResizeHandle.Visible = true end
    end
    function Window:Close()
        if ResizeHandle then ResizeHandle.Visible = false end
        Island.Visible = true
        Utility:TweenObject(Island, {Position = UDim2.new(0.5, 0, -0.08, 0)}, 0.2)

        Window.IslandOpen = false
        Window.Default = "Minimize"
        
        -- Fechamento suave e proporcional
        Utility:TweenObject(TabFrame, {Size = UDim2.new(0, Window.SideBarWidth, 0, 0)}, 0.2, Enum.EasingStyle.Quad, Enum.EasingDirection.Out)
        Utility:TweenObject(TabFrame, {BackgroundTransparency = 1}, 0.2, Enum.EasingStyle.Quad, Enum.EasingDirection.Out)
        Utility:TweenObject(LeftScroll, {Size = UDim2.new(1, 0, 0, 0)}, 0.2, Enum.EasingStyle.Quad, Enum.EasingDirection.Out)
        Utility:TweenObject(Main.Frame, {Size = UDim2.new(0, Window.Size.X.Offset, 0, 0)}, 0.2, Enum.EasingStyle.Quad, Enum.EasingDirection.Out)
        Main.Frame.Visible = false
        Utility:TweenObject(Main, {Size = UDim2.new(0, Window.Size.X.Offset, 0, 0)}, 0.2, Enum.EasingStyle.Quad, Enum.EasingDirection.Out)
        Utility:TweenObject(Main, {BackgroundTransparency = 1}, 0.2, Enum.EasingStyle.Quad, Enum.EasingDirection.Out)
        ...
        task.defer(function()
            ...
            task.wait(0.1)
            Utility:TweenObject(MinzUI, {Size = UDim2.new(0, MinzUI.UIListLayout.AbsoluteContentSize.X + 10, 0, 30)}, 0.2, Enum.EasingStyle.Quad, Enum.EasingDirection.Out)
        end)
        task.wait(0.2)
        Main.Visible = false
        MinzUI.Visible = true
            ...
            Buttons = {
                ...
                },
                {
                    ...
                    Callback = function()
                        ...
                        if TabFrame:FindFirstChild("Frame") then
