...
function UI:CreateWindow(Config)
    ...
    -- Sempre encostado no topo da tela (centro horizontal)
    ...
    -- Arrastar pelo handle (marca que o usuário moveu — não resetar no Close)
    ...
    do
        ...
        UserInputService.InputChanged:Connect(function(input)
            if dragging and (input.UserInputType == Enum.UserInputType.MouseMovement or input.UserInputType == Enum.UserInputType.Touch) then
                ...
                -- posição em offset de tela (estável em qualquer DPI / resolução)
        ...
        function Tab:Dropdown(Config)
            ...
            local function rebuildItemsInto(parentScroll)
                ...
                for idx, Item in ipairs(items) do
                    ...
                    local function isItemSelected()
                        if Dropdown.Multi then
                            for _, ov in ipairs(Dropdown.Options or {}) do
                                if ov == Item then return true end
                            end
                            return false
                        end
                    ...
                    local function applyRowVisual(isSel, instant)
                        local targetT = isSel and 0.15 or 1
                        local textT = isSel and 0 or 0.25
                        if instant then
                            row.BackgroundTransparency = targetT
                            label.TextTransparency = textT
                        else
                            Utility:TweenObject(row, { BackgroundTransparency = targetT }, 0.12, Enum.EasingStyle.Quad, Enum.EasingDirection.Out)
                            Utility:TweenObject(label, { TextTransparency = textT }, 0.12, Enum.EasingStyle.Quad, Enum.EasingDirection.Out)
                        end
                    end

                    row.MouseEnter:Connect(function()
                        if not isItemSelected() then
                            Utility:TweenObject(row, { BackgroundTransparency = 0.45 }, 0.08)
                        end
                    end)
                    row.MouseLeave:Connect(function()
                        applyRowVisual(isItemSelected(), true)
                    end)

                    -- flash claro ao pressionar; some ao soltar
                    row.InputBegan:Connect(function(input)
                        if Dropdown.Locked then return end
                        if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
                            Utility:TweenObject(row, { BackgroundTransparency = 0.05 }, 0.05)
                        ...
                        if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
                            task.defer(function()
                                if row and row.Parent then
                                    applyRowVisual(isItemSelected(), false)
                                end
                            end)
                        end
                    end)
                            ...
                            for _, r in ipairs(parentScroll:GetChildren()) do
                                if r:IsA("TextButton") then
                                    ...
                                    if lab then
                                        ...
                                        local targetT = isSel and 0.15 or 1
                                        local textT = isSel and 0 or 0.25
                                        Utility:TweenObject(r, { BackgroundTransparency = targetT }, 0.12)
                                        Utility:TweenObject(lab, { TextTransparency = textT }, 0.12)
                                    end
                                end
                            end
