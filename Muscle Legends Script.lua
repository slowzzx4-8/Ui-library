...
Tabs.Main:Toggle({
    Title = "Anti Stun",
    Default = cfgToggle("antiStun", State.fastSpeed == true),
    Callback = function(v) setFastSpeed(v); setCfgToggle("antiStun", v) end,
})
Tabs.Main:Toggle({
    Title = "Walk on Water",
...
Tabs.Main:Toggle({
    ...
    Callback = function(v) State.setAntiKnockback(v); setCfgToggle("antiKnockback", v) end,
})
...
for _, rock in ipairs(Config.Rocks) do
    ...
    local tog = Tabs.Glitch:Toggle({
        ...
        Callback = function(v)
            ...
            if v then
                if not State.fastPunch then
                    Window:Notify({ Title = "Features", Content = "Enable Fast Punch first", Icon = "hand", Duration = 2 })
                    return false
                end
...
-- restaura rock salva se fast punch estava on
task.defer(function()
    if State._savedRockName and cfgToggle("fastPunch", false) then
        for _, rock in ipairs(Config.Rocks) do
    ...
    if cfgToggle("antiStun", false) or State.fastSpeed then
        setFastSpeed(true)
    end
end)
