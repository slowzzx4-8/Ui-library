...
local function findRockByDurability(dur)
    ...
    if cached and typeof(cached) ~= "Instance" then
        cached = nil
    end
    ...
    if mf then roots[#roots + 1] = mf end
    local rocksFolder = workspace:FindFirstChild("Rocks") or workspace:FindFirstChild("rocks")
    if rocksFolder then roots[#roots + 1] = rocksFolder end
    roots[#roots + 1] = workspace
    for _, root in ipairs(roots) do
        for _, item in pairs(root:GetDescendants()) do
            if item.Name == "neededDurability" and item:IsA("ValueBase") then
                if tonumber(item.Value) == dur then
                    local parent = item.Parent
                    local rock = parent and (parent:FindFirstChild("Rock") or parent:FindFirstChild("rock"))
                    if not rock and parent and parent:IsA("BasePart") then
                        rock = parent
                    end
...
local function setFastPunch(enabled)
    ...
    -- não equipa tool; só acelera o attackTime se já existir Punch
...
-- Watermark: Muscle Legends | bolinha viva + ping
local function pingColorDot(ms)
    ms = tonumber(ms) or 0
    if ms <= 100 then
        return "🟢" -- boa (verde vivo)
    elseif ms <= 200 then
        return "🟡" -- média (amarelo/laranja vivo)
    else
        return "🔴" -- ruim (vermelho vivo)
    end
end
...
local function updateWatermark()
    ...
    pcall(function()
        Window:SetWatermark(string.format("Muscle Legends  |  %s %d ms", pingColorDot(ping), ping))
    end)
end
updateWatermark()
...
-- Config JSON (salva automático)
local CONFIG_FOLDER = "MuscleLegendsScript"

local function saveUserConfig()
    pcall(function()
        Window:SetConfig("mainSize", State.mainSize)
        Window:SetConfig("mainSpeed", State.mainSpeed)
        Window:SetConfig("fastSpeed", State.fastSpeed == true)
        Window:SaveConfig(CONFIG_FOLDER)
    end)
end

local function loadUserConfig()
    pcall(function()
        Window:LoadConfig(CONFIG_FOLDER)
        local size = Window:GetConfig("mainSize", nil)
        local speed = Window:GetConfig("mainSpeed", nil)
        if size ~= nil then State.mainSize = tonumber(size) or State.mainSize end
        if speed ~= nil then State.mainSpeed = tonumber(speed) or State.mainSpeed end
    end)
end
loadUserConfig()

-- autosave a cada 15s + ao fechar
...
for _, rock in ipairs(Config.Rocks) do
    ...
    local tog = Tabs.Glitch:Toggle({
        ...
        Callback = function(v)
            ...
            if v then
                if not State.fastPunch then
                    Window:Notify({ Title = "Features", Content = "Enable Fast Punch first", Duration = 2 })
                    return false
                end
                ...
                -- tenta achar a rock já
                pcall(function()
                    findRockByDurability(r.durability)
                    attachRockToHand()
                end)
...
Tabs.Kills:Toggle({
    ...
    Callback = function(v)
        if v and not State.spyTarget then
            Window:Notify({ Title = "View Player", Content = "Select a player first", Duration = 2 })
            return false
        end
        setSpy(v)
    end,
...
local function activeStatusText()
    ...
    if State.fastSpeed then parts[#parts + 1] = "Anti Stun" end
