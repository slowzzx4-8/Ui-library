...
local function startPunchTasks()
    ...
    cancelTask("fastPunchHit")
    cancelTask("fastPunchRock")
    -- equipa Punch (tool) continuamente — rocks + fast punch
...
State.clearAntiKnockbackStrong = function()
    if State.antiKnockbackStrongBV then
        State.antiKnockbackStrongBV:Destroy()
        State.antiKnockbackStrongBV = nil
    end
    if State.antiKnockbackStrongBG then
        State.antiKnockbackStrongBG:Destroy()
        State.antiKnockbackStrongBG = nil
    end
    cancelTask("antiKnockbackStrong")
end

-- clássico (usado no Anti Stun)
...
-- forte: personagem nunca é jogado longe
State.setAntiKnockbackStrong = function(enabled)
    State.antiKnockbackStrong = enabled == true
    if not State.antiKnockbackStrong then
        State.clearAntiKnockbackStrong()
        return
    end
    cancelTask("antiKnockbackStrong")
    spawnTask("antiKnockbackStrong", function()
        while State.running and State.antiKnockbackStrong do
            local hrp = getHRP()
            local hum = getHumanoid()
            if hrp and hum and not State.fly then
                if not State.antiKnockbackStrongBV or State.antiKnockbackStrongBV.Parent ~= hrp then
                    State.clearAntiKnockbackStrong()
                    State.antiKnockbackStrongBV = Instance.new("BodyVelocity")
                    State.antiKnockbackStrongBV.Name = "AntiKnockbackStrong"
                    State.antiKnockbackStrongBV.P = 50000
                    State.antiKnockbackStrongBV.MaxForce = Vector3.new(1e11, 1e11, 1e11)
                    State.antiKnockbackStrongBV.Parent = hrp
                    State.antiKnockbackStrongBG = Instance.new("BodyGyro")
                    State.antiKnockbackStrongBG.Name = "AntiKnockbackStrongGyro"
                    State.antiKnockbackStrongBG.P = 30000
                    State.antiKnockbackStrongBG.MaxTorque = Vector3.new(1e11, 1e11, 1e11)
                    State.antiKnockbackStrongBG.CFrame = hrp.CFrame
                    State.antiKnockbackStrongBG.Parent = hrp
                end
                local md = hum.MoveDirection
                local spd = State.fastSpeed and 1000 or math.max(hum.WalkSpeed, 16)
                local vy = hrp.AssemblyLinearVelocity.Y
                -- limita Y (sem ser jogado pro alto) e segura horizontal
                if math.abs(vy) > 80 then
                    vy = math.clamp(vy, -40, 40)
                end
                State.antiKnockbackStrongBV.Velocity = Vector3.new(md.X * spd, vy * 0.15, md.Z * spd)
                State.antiKnockbackStrongBG.CFrame = CFrame.new(hrp.Position, hrp.Position + hrp.CFrame.LookVector)
                hrp.AssemblyLinearVelocity = Vector3.new(md.X * spd, math.clamp(hrp.AssemblyLinearVelocity.Y, -35, 35), md.Z * spd)
                hrp.AssemblyAngularVelocity = Vector3.zero
            end
            RunService.Heartbeat:Wait()
        end
...
local function shutdown()
    ...
    if State.setAntiKnockbackStrong then State.setAntiKnockbackStrong(false) end
    setNoclip(false)
    setSpin(false)
    setSpy(false)
    State.stopKills()
...
Tabs.Main:Toggle({
    ...
    Default = cfgToggle("antiKnockback", false),
    Callback = function(v)
        State.setAntiKnockbackStrong(v)
        setCfgToggle("antiKnockback", v)
    end,
})
...
local function activeStatusText()
    ...
    if State.antiKnockbackStrong then parts[#parts + 1] = "Anti KB" end
...
task.defer(function()
    ...
    if cfgToggle("antiKnockback", false) then
        State.setAntiKnockbackStrong(true)
    end
end)
